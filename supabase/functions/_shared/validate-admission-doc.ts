// Helper compartilhado: validação IA de um documento de admissão.
//
// Extraído de admission-document-validate pra ser reusado pelo
// admission-public-submit em background (auto-trigger logo após upload).
//
// O caller é responsável por:
//   - Fornecer um Supabase client com privilégios de service_role.
//   - Garantir as permissões antes de chamar (auth + role + empresa).
//
// Esta função NÃO faz auth — é só a lógica de download → Claude → persist.

import type { SupabaseClient } from "https://esm.sh/@supabase/supabase-js@2.49.0";
import { callClaude, extractTextFromResponse } from "./claude.ts";
...
export interface RunValidationOptions {
  ...
  documentId: string;
  /** UUID do user que disparou. null = sistema (auto-trigger no submit). */
...
/**
 * Roda o pipeline completo: fetch doc + journey, baixa arquivo, chama Claude,
 * persiste em admission_documents.ai_validation_result + ai_confidence,
 * insere evento doc_validated.
 *
 * Throw `ValidationError` em casos esperados (404, 400, 409); throw genérico
 * em falhas de IA / DB. O caller decide o que fazer com isso.
 */
export async function runDocumentValidation(
  opts: RunValidationOptions,
): Promise<RunValidationResult> {
  const { sbAdmin, documentId, actorUserId } = opts;

  // 1. Doc + journey
  const { data: doc, error: docErr } = await sbAdmin
    .from("admission_documents")
    .select("id, company_id, journey_id, doc_type, file_url, status")
    .eq("id", documentId)
    .single();

  if (docErr || !doc) {
    throw new ValidationError(404, "Documento não encontrado");
  }
  if (!doc.file_url) {
    throw new ValidationError(
      400,
      "Documento ainda não foi enviado pelo candidato",
    );
  }
  if (doc.status === "approved") {
    throw new ValidationError(
      409,
      "Documento já aprovado — validar de novo não faz sentido",
    );
  }
  ...
  if (jErr || !journey) {
    throw new ValidationError(404, "Journey não encontrada");
  }

  // 2. Marca status como ai_validating
  await sbAdmin
    .from("admission_documents")
  ...
  try {
    ...
    if (dlErr || !blob) {
      throw new Error(dlErr?.message ?? "blob vazio");
    }
    ...
    await sbAdmin
      ...
      .eq("id", documentId);
    throw new Error("Storage download: " + (err as Error).message);
  }
...
Valide e retorne o JSON conforme o schema.`;
  ...
  try {
    ...
    await sbAdmin
      ...
      .update({ status: "submitted" })
      .eq("id", documentId);
    throw new Error("Claude: " + (err as Error).message);
  }

  // 6. Persiste + volta status pra submitted
  ...
  if (updateErr) {
    throw new Error("DB persist: " + updateErr.message);
  }

  // 7. Evento timeline
  const summaryMsg = buildEventMessage(
    doc.doc_type,
    aiResult,
    actorUserId === null,
  );
  ...
  return { result: aiResult, confidence };
}
...
function buildEventMessage(
  ...
  if (!r.type_matches) {
    parts.push(`tipo divergente (detectou: ${r.detected_type})`);
  }
  if (!r.data_matches_cadastro) parts.push("dados não batem com cadastro");
  const issues = parts.length > 0 ? ` — ${parts.join(", ")}` : " — sem ressalvas";
  const prefix = isAutoTrigger
    ? `IA validou "${docType}" automaticamente após upload`
    : `IA validou "${docType}"`;
  return `${prefix} (confiança ${(r.confidence * 100).toFixed(0)}%)${issues}`;
}
